class UsersController < ApplicationController
  ...
  def params_present
    @five_record = FiveKmRecord.find_by(params[:user_id]) if FiveKmRecord.where(params[:user_id]).present?
    @ten_record = TenKmRecord.find_by(params[:user_id]) if TenKmRecord.where(params[:user_id]).present?
    @half_record = HalfRecord.find_by(params[:user_id]) if HalfRecord.where(params[:user_id]).present?
    @full_record = FullRecord.find_by(params[:user_id]) if FullRecord.where(params[:user_id]).present?
  end

  def set_record
    if FiveKmRecord.where(params[:id]).present?
      five_record = FiveKmRecord.find_by(params[:user_id])
      @five_minute = five_record.minute[:name].to_i
      @five_second = five_record.second[:name].to_i
    end

    if TenKmRecord.where(params[:id]).present?
      ten_record = TenKmRecord.find_by(params[:user_id])
      @ten_minute = ten_record.minute[:name].to_i
      @ten_second = ten_record.second[:name].to_i
    end

    if HalfRecord.where(params[:d]).present?
      half_record = HalfRecord.find_by(params[:user_id])
      @half_hour = half_record.hour[:name].to_i
      @half_minute = half_record.minute[:name].to_i
      @half_second = half_record.second[:name].to_i
    end

    if FullRecord.where(params[:id]).present?
      full_record = FullRecord.find_by(params[:user_id])
  ...
  def user_half_record
    if HalfRecord.where(params[:id]).present?
